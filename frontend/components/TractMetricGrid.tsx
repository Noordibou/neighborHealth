...
export function TractMetricGrid({ tract }: { tract: TractDetail }) {
      ...
      {METRIC_KEYS.map((key) => {
        ...
        return (
          <MetricCard
            ...
            valueLabel={formatMetricValue(key, ind?.value ?? null)}
            statePct={ind?.percentile_state ?? null}
            nationalPct={ind?.percentile_national ?? null}
            icon={iconFor(key)}
          />
        );
      })}
